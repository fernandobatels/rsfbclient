...
fn main() {
    ...
    let stmt = tr.prepare("select col_a, col_b, col_c from test".to_string())
    ...
    loop {
        ...
        let row_op = rows.fetch()
            .expect("Error on fetch the row");

        if let Some(row) = row_op {
            let col_a: i32 = row.get(0)
                .expect("Error on get the value from 1° column");
            let col_b: f32 = row.get(1)
                .expect("Error on get the value from 2° column");
            let col_c: String = row.get(2)
                .expect("Error on get the value from 3° column");
